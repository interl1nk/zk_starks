...
class MerkleTree(object):
    """
    Простая и наивная реализация неизменяемого дерева Меркла.
    """
    ...
    def get_authentication_path(self, leaf_id):
        ...
        cur = self.root
        decommitment = []
        # В дереве Меркла путь от корня к листу соответствует идентификатору листа.
        # двоичному представлению, начиная со второго-MSB, где '0' означает 'левый', а '1' -
        # 'правый'.
        # Поэтому мы перебираем биты двоичного представления, пропуская префикс '0b'
        # префикс, а также MSB.
