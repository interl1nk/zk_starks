...
class FieldElement:
    """
    Представляет собой элемент F_(3 * 2**30 + 1).
    """
    k_modulus = 3 * 2**30 + 1
    generator_val = 5
    ...
    def one():
        """
        Получает единичный элемент поля.
        """
        return FieldElement(1)

    def __repr__(self):
        # Выберите более короткое представление между положительными и отрицательными значениями элемента.
    ...
    def typecast(other):
        ...
        assert isinstance(other, FieldElement), f'Несоответствие типов: FieldElement и {type(other)}.'
        return other
    ...
    def is_order(self, n):
        """
        Наивно проверяет, что элемент имеет порядок n, возводя его во все степени до n, проверяя, что
        что элемент до n-ой степени является единицей, но не так для любого k<n.
        """
        assert n >= 1
        h = FieldElement(1)
        for _ in range(1, n):
